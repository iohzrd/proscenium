...
use proscenium_types::{
    SignedStageControl, StageControl, StageEvent, StageRole, StageState, StageTicket, now_millis,
    short_id, sign_stage_control,
};
...
pub(super) struct SfuHub {
    ...
    pub(super) fanouts: std::collections::HashMap<String, Arc<Fanout>>,
    /// QUIC node-id -> their live Connection (for opening new uni-streams).
    pub(super) connections: std::collections::HashMap<String, iroh::endpoint::Connection>,
}
...
struct ActiveStage {
    ...
    speaker_mixer: Option<speaker_mixer::SpeakerMixerHandle>,
